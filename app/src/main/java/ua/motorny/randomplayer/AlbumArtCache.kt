...
import ua.motorny.randomplayer.utils.BitmapHelper
...
class AlbumArtCache private constructor() {
    ...
    fun fetch(artUrl: String, listener: FetchListener) {
        ...
        object : AsyncTask<Void, Void, Array<Bitmap>>() {
            override fun doInBackground(objects: Array<Void>): Array<Bitmap>? {
                ...
                try {
                    val tempBitmap = BitmapHelper.fetchAndRescaleBitmap(artUrl,
                            MAX_ART_WIDTH, MAX_ART_HEIGHT)
                    val icon = BitmapHelper.scaleBitmap(tempBitmap,
                            MAX_ART_WIDTH_ICON, MAX_ART_HEIGHT_ICON)
                    bitmaps = arrayOf(tempBitmap, icon)
    ...
    companion object {
        private val TAG = LogHelper.makeLogTag(AlbumArtCache::class)
